...
Nothing is hand-transcribed (CLAUDE.md 4-3).
  ...
  EMBED  a 2x2 t-SNE grid: rows are the two blocks, columns are the two factors.
         This is the block x factor matrix made visible -- the same three terms
         SEP multiplies (inclusion / allocation / exclusion), one per panel.

  TRAJ   z_slow's leading principal components against time, with the regime
         switches marked. If the block tracks a persistent factor it should sit
         still between switches and move at them.

HONEST READING (do not oversell the grid). z_mix is NOT the mirror image of
z_slow. The gate only constrains when z_mix is available to the predictor, not
what it contains, and near-horizon prediction needs the current persistent state
as much as the transient one -- so z_mix carries BOTH factors by design
(CLAUDE.md 2). Three of the four panels carry a claim; the z_mix/persistent panel
is expected to show structure and is not evidence against us.
...
def fig_traj(enc, tag, lam, n_samp=90_000, stride_patches=4, seed=7):
    ...
    fig.suptitle(f"$z_{{slow}}$ moves at regime switches and sits still between them "
