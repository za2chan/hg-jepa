...
generative process: fast dynamics are \emph{conditioned on} the slow state
(the next gait sample needs the current activity; the next oscillation sample
needs the regime's carrier), so short-horizon targets need both blocks
($I(\text{near};s_t\mid f_t)>0$), whereas the fast factor forgets itself
within $T_{\mathrm{ac}}$ ($I(\text{far};f_t\mid s_t)\approx 0$). A gate
should remove only \emph{conditionally useless} information: closing the fast
path at long horizons is harmless, but the symmetric variant (closing the
slow path at short horizons) would cut information the short-horizon loss
still needs, forcing the encoder to \emph{copy} slow information into
...
encoder's smoothness bias (a bounded-Lipschitz network does not realize the
wild injection, only approximates it --- so exclusion is empirical, cf.\ F4).
A uniqueness theorem, and Prop.~1's condition that regimes differ in the
\emph{mean} (not only in variance), remain open.
...
report \emph{absolute} scores (accuracy / $R^2$ / macro-F1), not the leak
\emph{ratios} of earlier iterations (which once hid a collapse), so all
numbers here are lower and non-comparable. RankMe \cite{garrido2023rankme}
monitors collapse; $n{=}3$ seeds, one backbone and budget for all variants;
one patch is 8 steps, so horizons $\{1{,}4{,}16{,}64{,}128\}$ span
...
only weakly (slow-block $0.63$, full $0.54$) --- reconstruction is local, so a
factor needing integration over hundreds of steps barely enters its code.
