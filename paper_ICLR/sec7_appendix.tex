...
\section{Where the persistence--slowness coincidence holds}
\label{app:coincidence}

Section~\ref{sec:rotation} attributes SFA's lead on real data to persistence and slowness coinciding there. That is an explanation, and this appendix turns it into two measurements, because the word ``slow'' was carrying two meanings at once: \textbf{(a)} the factor is legible in the low-frequency band of the signal, and \textbf{(b)} some direction of the representation changes little over time. SFA optimizes (b) --- under a unit-variance constraint it minimizes the time derivative of its output, which says nothing about where in the spectrum the factor lives.

For (a) we filter the raw signal to one band at a time and fit the same linear probe on that band alone (Table~\ref{tab:bandaccess}). Only HAPT clears chance by a real margin. On PTB-XL the low band is \emph{at} chance and the diagnosis lives instead in the 2--5\,Hz QRS morphology --- yet SFA wins on PTB-XL, so sense (a) is not what is happening there.

\input{tab_bandaccess}
...
\begin{figure}[!tbp]
\centering
\includegraphics[width=0.82\textwidth]{figs/fig_embed_nomech_nce.png}
\caption{The same view as Fig.~\ref{fig:embed} for a model trained with no mechanism at all --- no gate, no penalty, no per-block LayerNorm. The persistent factor is visible in the rear block here too, which is why its visibility in Fig.~\ref{fig:embed} is not evidence that our devices failed to remove it: they never act on it.}
\label{fig:embed_nomech}
\end{figure}
