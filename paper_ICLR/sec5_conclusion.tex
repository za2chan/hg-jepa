...
Two measurements sharpen that comparison. Splitting SEP into its three factors
shows that the shortfall against SFA is confined to the exclusion term --- the
one term the objective cannot enforce --- while inclusion is a draw everywhere;
and that shortfall is not an artifact of linear probing, since it survives and
in places widens under a non-linear reader that also collapses our own
exclusion. On the datasets where SFA leads, two different mechanisms are at
work: on HAPT the persistent factor really does sit in the low band of the
signal, whereas on PTB-XL it does not but is constant within a window, which is
the limiting case of what SFA maximizes. Finally, the coefficient that governs
exclusion can be selected without task labels, agreeing with the label-based
choice in all six settings.
...
\textbf{First, the exclusion we demonstrate is a linear-level one.} The
objective enforces availability but not exclusion, and Section~\ref{sec:exclusion}
shows that this is not a formality: swapping the linear probe for a small MLP,
with the same head given to every baseline, leaves inclusion and allocation
intact and collapses exclusion alone, by $\numMlpExcl$ on average and
$\numMlpExclWorst$ at worst. The transient factor is not absent from $\zslow$;
it is linearly unreadable there. A structural repair does not help either --- a
gate that also switches $\zslow$ off below $\tau$ is worse in $\numSymWorse$ of
six settings. What we can claim is that the method fails where its own analysis
said it would, and nowhere else.

\textbf{Second, most hyperparameters cannot be chosen without labels, but the
one that governs exclusion can.} The block width, the target width, the patch
size and the horizon range have no derivation rule and were set by downstream
performance; model selection without labels is a known difficulty in this
literature \cite{locatello2019}. The cross-covariance coefficient $\lambda$ is
now an exception: the label-free criterion $J$ of Eq.~\ref{eq:J} picks the
label-selected value in all six settings. That agreement is decisive only on
synthetic data, where the gap to the runner-up is
$\numJmarginSynthLo$--$\numJmarginSynthHi$; on real data it is
$\numJmarginRealLo$--$\numJmarginRealHi$ and we did not store the seed-level
spread, so those four agreements are not distinguishable from chance. On a
fourth dataset not reported here, the criterion selected a different $\lambda$
from the label-based one; that analysis is unfinished --- one configuration axis
was never tested --- and we report neither its results nor its disagreement as
evidence. The autocorrelation-derived horizon threshold remains a partial
exception in the other direction: it fails on regularly repeating signals, since
on PTB-XL the per-record estimates scatter from $0.1$ to $49.9$ patches and the
rule returns $56.6$, outside the training horizon range, so we set the threshold
by hand instead.

\textbf{Third, we did not analyze the target width.} We used 8, 12, and 8
...
\textbf{Fourth, we did not demonstrate a practical benefit:} the perturbed-domain effect clears the error bars in only one of six settings, and even there absolute performance never overtakes the mechanism-free embedding.

\textbf{Fifth,} the claim that a factor can be ``persistent yet absent from
...
\textbf{Sixth, our axis selects persistence, not relevance.} A persistent
