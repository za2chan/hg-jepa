...
We use the synthetic generator of Section~\ref{sec:synthetic} and two public sets, PTB-XL \citep{wagner2020} and HAPT \citep{reyesortiz2016, hapt2015}.

\begin{table}[!tb]
\caption{The three datasets. The transient proxy is computed from the signal, never from labels.}
\label{tab:datasets}
\centering
\footnotesize
...
\hline
\multicolumn{7}{l}{Channels 1 / 1 (lead II) / 3 (accel.); groups --- / 4,812 patients / 30 subjects; chance 0.333 / 0.500 / 0.167.}
\end{tabular}
\end{table}

Three choices the table cannot carry. \textbf{Splits are per group} --- patient for PTB-XL, subject for HAPT --- and normalization uses training-split statistics only. \textbf{Nothing is selected by label before training}: HAPT's transition and unlabeled segments, 24.7\% of the total, are excluded from scoring but left in the pre-training input, since filtering them out would undermine the claim that training uses no labels; windows are likewise cut without consulting activity boundaries, and labels are read per position. And PTB-XL strains the rule $\tau$ comes from --- the heartbeat repeats regularly, so its autocorrelation oscillates instead of decaying and no lifetime can be read from it, and $\tau$ is set by hand there (Section~\ref{sec:limits}).
