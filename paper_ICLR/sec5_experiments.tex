...
Every setting below is one of six: three datasets crossed with the two loss stems, at $n=5$ seeds unless stated.
...
\begin{table}[!htb]
\caption{The three datasets. The transient proxy is computed from the signal, never from labels. Channels are 1, 1 (lead II) and 3 (acceleration axes); the grouping unit for splits is 4,812 patients for PTB-XL and 30 subjects for HAPT.}
\label{tab:datasets}
\centering
\footnotesize
...
\textbf{Probe protocol.} With the encoder frozen we fit only a linear classifier or regressor. Splits are \textbf{per group} --- patient for PTB-XL, subject for HAPT, a front/back time split for the synthetic data --- evaluation windows are \textbf{non-overlapping} to prevent leakage (asserted in the code), and only positions after the \textbf{minimum context length (64 patches)} are scored, since with less context the encoder lacks the evidence to read the persistent state. A \textbf{random subspace} of the same width is scored as a control. No data is selected by label before training: HAPT's transition and unlabeled segments, 24.7\% of the total, are excluded from scoring but left in the pre-training input, windows are cut without consulting activity boundaries, and labels are read per position.

On PTB-XL the rule that initializes $\tau$ does not apply, since the heartbeat repeats regularly and its autocorrelation oscillates instead of decaying; $\tau$ is set by hand there (Section~\ref{sec:limits}).
...
% Two tables in one float: each minipage carries its own \caption, so they keep
% their own numbers (2 and 3) and their own \label, and no subfigure package is
% needed. Declared here in 5.2; 5.3 refers back to the right-hand one.
\begin{table}[!htb]
...
\begin{tabular}{lcccc}
\hline
Setting & Neither & Gate & xcov & \textbf{Both} \\
\hline
Synth. / Reg & 0.135 & 0.342 & 0.279 & \textbf{0.408} \\
Synth. / NCE & 0.121 & 0.124 & 0.488 & \textbf{0.749} \\
...
The ``Neither'' cells take the first 16 of the 64 dimensions, and their SEP agrees in all six settings to within 0.015 with the SEP of 16 dimensions drawn at random from the same embedding. Without our devices, any part of the embedding holds the two factors to a similar degree, so coordinate position carries no meaning by itself; our contribution is to give those coordinates structure. Fig.~\ref{fig:embed_full} in Appendix~\ref{app:embed} shows the same contrast qualitatively, as a t-SNE of the two blocks with the mechanism and without it.

Three further design choices could each have gone the other way. Replacing the smooth gate with a step at $\tau$, gating $\zslow$ symmetrically as well, and predicting the raw signal instead of a latent target are each tested in Appendix~\ref{app:design}; all three are worse, and the latent target by a factor of two.
...
\begin{figure}[!htb]
\centering
\includegraphics[width=0.78\textwidth]{figs/fig_rotation_sep.png}
