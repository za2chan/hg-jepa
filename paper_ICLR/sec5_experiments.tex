...
\begin{figure}[!tbp]
\centering
\includegraphics[width=0.82\textwidth]{figs/fig_embed_nce_lam4.png}
\caption{The three SEP terms, seen directly. A t-SNE of each block of one trained embedding (synthetic $\pm$3\%, HGLP-NCE, $\lambda=4$), coloured by each factor; the four panels are the three terms of Eq.~\ref{eq:sep} plus the one quantity we do not constrain. \textbf{Top left, inclusion}: the three regimes are separated in $\zslow$. \textbf{Top right, exclusion}: the transient factor is not organized there. \textbf{Bottom right, allocation}: it is organized in $\zmix$ instead. \textbf{Bottom left} is the panel to read carefully --- the regimes are visible in $\zmix$ as well, which is expected rather than a failure: the gate restricts when $\zmix$ may be read, never what it may contain (Section~\ref{sec:asymmetry}). A mechanism-free model shows the same panel (Fig.~\ref{fig:embed_nomech}), so it is not something our devices act on.}
\label{fig:embed}
\end{figure}
...
The explanation is that persistence and slowness mostly coincide on real data, and Appendix~\ref{app:coincidence} measures in what sense. Two senses have to be told apart: whether the factor is legible in the low-frequency band of the \emph{signal}, and whether some direction of the \emph{representation} changes little over time --- SFA optimizes the second. On HAPT the first holds, at $\numHaptBandRatio\times$ chance, because posture is the DC distribution of gravity across three axes. On PTB-XL it does not: the low band is at chance (\numPtbLowBand{} against \numPtbChance) and the diagnosis lives in the QRS morphology --- but the diagnosis is constant over a record, so its time derivative is zero, which is the limiting solution of what SFA maximizes. HAPT and PTB-XL are won by different routes, and the synthetic data, where the factor is neither band-slow nor window-constant, is closed to both. What remains ours is that the coordinates are fixed during training, so no second fitting stage is needed at deployment --- an operational difference, not a difference in the quality of the separation.
