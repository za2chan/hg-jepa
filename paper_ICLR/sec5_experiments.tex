...
\caption{\textbf{The three terms of SEP, with and without the mechanism.} A t-SNE of one block of one trained embedding per panel (synthetic $\pm$3\%, HGLP-NCE, $\lambda=4$), coloured by one factor; the three columns are exactly the three terms of Eq.~\ref{eq:sep} and the bottom row is a model trained with no gate, no penalty and no per-block LayerNorm. \textbf{Inclusion}: the three regimes separate in $\zslow$ in both rows --- which is the point of the random-subspace control above, not a result. \textbf{Exclusion}: the transient factor is unstructured in $\zslow$ with the mechanism and structured without it. \textbf{Allocation}: it is organized in $\zmix$ instead. The fourth combination, $\zmix$ coloured by the persistent factor, is not a term of SEP and is unconstrained by design (Section~\ref{sec:asymmetry}), so it is omitted.}
\label{fig:embed}
\end{figure}
